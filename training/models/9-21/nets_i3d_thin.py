...
class Mixed_3b(nn.Module):
    def __init__(self):
        ...
        self.branch2 = nn.Sequential(
            BasicConv3d(96, 8, kernel_size=1, stride=1),
            BasicConv3d(8, 16, kernel_size=3, stride=1, padding=1),
        )
...
class Mixed_4b(nn.Module):
    def __init__(self):
        ...
        self.branch0 = nn.Sequential(
            BasicConv3d(240, 96, kernel_size=1, stride=1),
        )
        self.branch1 = nn.Sequential(
            BasicConv3d(240, 48, kernel_size=1, stride=1),
            BasicConv3d(48, 104, kernel_size=3, stride=1, padding=1),
        )
        self.branch2 = nn.Sequential(
            BasicConv3d(240, 8, kernel_size=1, stride=1),
            BasicConv3d(8, 24, kernel_size=3, stride=1, padding=1),
        )
        self.branch3 = nn.Sequential(
            nn.MaxPool3d(kernel_size=(3, 3, 3), stride=1, padding=1),
            BasicConv3d(240, 32, kernel_size=1, stride=1),
        )
...
class Mixed_4c(nn.Module):
    def __init__(self):
        ...
        self.branch1 = nn.Sequential(
            ...
            BasicConv3d(56, 112, kernel_size=3, stride=1, padding=1),
        )
        self.branch2 = nn.Sequential(
            BasicConv3d(256, 12, kernel_size=1, stride=1),
            BasicConv3d(12, 32, kernel_size=3, stride=1, padding=1),
        )
        self.branch3 = nn.Sequential(
            nn.MaxPool3d(kernel_size=(3, 3, 3), stride=1, padding=1),
...
class I3D_S_thin(nn.Module):

    def __init__(self, num_classes=400, input_channel=3, dropout_keep_prob=0.0):
        super(I3D_S_thin, self).__init__()
        self.features = nn.Sequential(
            BasicConv3d(input_channel, 32, kernel_size=7, stride=2, padding=3), # (64, 32, 112, 112)
            nn.MaxPool3d(kernel_size=(1,3,3), stride=(1,2,2), padding=(0,1,1)),  # (64, 32, 56, 56)
            BasicConv3d(32, 32, kernel_size=1, stride=1), # (64, 32, 56, 56)
            BasicConv3d(32, 96, kernel_size=3, stride=1, padding=1),  # (192, 32, 56, 56)
        ...
        self.classifier = nn.Conv3d(256, num_classes, kernel_size=1, stride=1, bias=True)
